...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/xamarin-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/xamarin-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '' && changedFileName !== '.gitignore' && !changedFileName.includes('.gitlab')) {
                ...
                if (fs.existsSync('../xamarin-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/xamarin-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../xamarin-docs/` + changedFileName, `../../../gitlabRepo/xamarin-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/xamarin-docs/')) {
                            shelljs.cp('-rf', `../xamarin-docs/` + changedFileName, `../../../gitlabRepo/xamarin-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/xamarin-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/xamarin-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/xamarin-docs`);
